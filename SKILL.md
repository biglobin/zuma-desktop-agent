...
download_url: https://gitee.com/biglobin/zuma-desktop-agent  # 用于下载或更新
homepage: https://github.com/biglobin/zuma-desktop-agent     # 项目主页（可用于下载或更新）
---
...
本 Skill 放入 **你的 skills 目录** 后**不会自动启用**，必须在 OpenClaw 配置中显式开启。

> **不同发行版的 skills 目录位置**：
> - QClaw: `~/.qclaw/skills/`
> - OpenClaw: `~/.openclaw/skills/`
> - ArkClaw: `~/.arkclaw/skills/`
> - 其他：查看你的配置文件中 `skills.directories` 设置

### 安装步骤

1. **克隆/放置**到你的 skills 目录下的 `zuma-desktop-agent/` 文件夹
2. **安装依赖**：`npm install`
3. **【关键】启用 Skill**：
   使用你的 OpenClaw 配置工具设置（命令因发行版而异）：

   | 发行版 | 配置命令 |
   |--------|---------|
   | QClaw | `qclaw gateway config.patch` 或 Web UI |
   | OpenClaw | `openclaw gateway config.patch` 或 Web UI |
   | ArkClaw | `arkclaw gateway config.patch` 或 Web UI |
   ...
   ```
4. **重启 Gateway**（命令：`qclaw gateway restart` / `openclaw gateway restart` 等）
5. **验证**：使用对应命令（如 `qclaw gateway config.get`）确认 `skills.entries.zuma-desktop-agent.enabled === true`

### 常见问题

**Q: 安装后发送触发词没有反应？**
A: 检查配置中 `enabled` 是否为 `true`。这是最常见的问题。
